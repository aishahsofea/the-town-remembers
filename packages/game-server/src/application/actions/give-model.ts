/**
 * Production Bedrock implementation behind `give`'s injected dialogue seam
 * (`P4-15`). Structurally identical to `tell-model.ts`'s `selectDialogue` —
 * one dialogue-selection call, one repair attempt, then the authored
 * fallback — since `give`, like `tell`, has no player query to embed and no
 * disclosure selection of its own: every causal effect is already decided by
 * `planGive` before this seam ever runs.
 */
...
import {
  ...
  type GiveActionDependencies,
  type GiveDialogueSelectionParams,
} from "./inputs/give.js";

export interface ProductionGiveDependenciesParams {
  readonly pool: Pool;
...
function fallbackSelection(params: GiveDialogueSelectionParams): GiveDialogueSelection {
  const gateResult = params.assembled.trustedContext.dialogue_directive.gate_result;
  // Custody actually transferring requires the outcome-specific "I have it
  // now." line (docs/009 "Successful mechanical outcomes use exact
  // outcome-specific fallbacks"), never the generic refusal-flavored line —
  // the two are keyed by different `responseKind`s in the authored corpus.
  const line = resolveFallbackLine(BELL_CONTENT, {
    npcKey: params.npcKey,
    actionKind: "give",
    responseKind: params.npcAcceptsItem ? "acknowledge" : "refuse",
    gateResult,
    requiredOutcomeIds: params.npcAcceptsItem ? ["requested_item_received"] : [],
  });
  return {
    npcId: params.assembled.trustedContext.npc_profile.npc_id,
    text: line.text,
...
async function recordStructuredCall(params: {
  readonly dependencies: ProductionGiveDependenciesParams;
  readonly request: GiveDialogueSelectionParams;
...
async function selectDialogue(
  ...
  if (outcome.kind === "accepted") {
    ...
    if (validation.valid) {
      ...
      return toGiveSelection(validation, params, "selected");
    }
    failure = validation;
  ...
  if (repairOutcome.kind === "accepted") {
    ...
    if (validation.valid) {
      ...
      return toGiveSelection(validation, params, "repaired");
    }
    await recordStructuredCall({
      dependencies,
      request: params,
...
export function createProductionGiveDependencies(
  params: ProductionGiveDependenciesParams,
): GiveActionDependencies {
  return {
...
export function createProductionGiveActionHandler(params: {
  readonly pool: Pool;
  readonly modelConfig: ModelConfig;
  readonly now?: () => Date;
}) {
  const dependencies = createProductionGiveDependencies({
    ...params,
    converseClient: createBedrockConverseClient(params.modelConfig.region),
  });
  return createGiveHandler(dependencies);
}

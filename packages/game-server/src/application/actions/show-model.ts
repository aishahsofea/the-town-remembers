/**
 * Production Bedrock implementation behind `show`'s injected dialogue seam
 * (`P4-14`). Structurally identical to `tell-model.ts`'s `selectDialogue` —
 * one dialogue-selection call, one repair attempt, then the authored
 * fallback — since `show`, like `tell`, has no player query to embed and no
 * disclosure selection of its own: every causal effect is already decided by
 * `planShow` before this seam ever runs.
 */
...
import type { ShowDialogueSelection } from "@the-town-remembers/rules";
...
import {
  createShowActionHandler as createShowHandler,
  type ShowActionDependencies,
  type ShowDialogueSelectionParams,
} from "./inputs/show.js";

export interface ProductionShowDependenciesParams {
  readonly pool: Pool;
...
function toShowSelection(
  validation: DialogueValidationSuccess,
  params: ShowDialogueSelectionParams,
...
function fallbackSelection(params: ShowDialogueSelectionParams): ShowDialogueSelection {
...
async function reserveChatCall(
  dependencies: ProductionShowDependenciesParams,
  params: ShowDialogueSelectionParams,
...
async function recordStructuredCall(params: {
  readonly dependencies: ProductionShowDependenciesParams;
  readonly request: ShowDialogueSelectionParams;
...
async function selectDialogue(
  ...
  if (outcome.kind === "accepted") {
    ...
    if (validation.valid) {
      ...
      return toShowSelection(validation, params, "selected");
    }
    failure = validation;
  ...
  if (repairOutcome.kind === "accepted") {
    ...
    if (validation.valid) {
      ...
      return toShowSelection(validation, params, "repaired");
    }
    await recordStructuredCall({
      dependencies,
      request: params,
...
export function createProductionShowDependencies(
  params: ProductionShowDependenciesParams,
...
export function createProductionShowActionHandler(params: {
  readonly pool: Pool;
  readonly modelConfig: ModelConfig;
  readonly now?: () => Date;
}) {
  const dependencies = createProductionShowDependencies({
    ...params,
    converseClient: createBedrockConverseClient(params.modelConfig.region),
  });
  return createShowHandler(dependencies);
}

...
async function freezeTownForResolution(
  ...
  await pool.query(
    ...
    [townId, eventId, bumped.rows[0]!.last_event_sequence, now, `test:freeze:${eventId}`, ownerPlayerId],
  );
...
describe.skipIf(!shouldRunDatabaseTests())("POST /actions — start_visit and travel", () => {
  ...
  beforeAll(async () => {
    ...
    if (otherResult.outcome !== "committed") throw new Error("The seed did not commit.");
  ...
  it("acceptance 4: a cross-town location, an item id, and a random UUID all produce an identical DESTINATION_UNKNOWN denial", async () => {
    ...
    expect(
      (bodies[0] as { result: { reasonCode: string } }).result.reasonCode,
    ).toBe("DESTINATION_UNKNOWN");
  }, 30_000);
  ...
  it("acceptance 6: a successful travel emits exactly one travelled event and updates the visit's location in the same transaction as the revision bump", async () => {
    ...
    expect(revisionAfter.rows[0]!.revision).toBe(revisionBefore.rows[0]!.revision + 1);
...
describe.skipIf(!shouldRunDatabaseTests())("POST /actions — generic executor outcome wiring", () => {
  let handle: DisposableDatabase | undefined;
  let config: RouterConfig;
  let townId: string;
  let inviteToken: string;
